...
class TREEYA:
    ...
    async def call_search_stock(self, prev_contents, TTT_response):
        ...
        t1 = time.time()
        response1, type_ = await self.TTT(contents)
        t2 = time.time()
        print(f"Time taken: TTT: {(t2-t1)*1000:2f} ms")
        
        prev_contents = await self.add_content(prev_contents, "user", response)
...
if __name__ == '__main__':
    uvicorn.run("main_progress:app", host = "localhost", port = 8000, reload = True)